...
/// Tree-sitter backed language implementation for TypeScript/TSX.
...
impl TypeScriptBackend {
    fn select_language(&self, path: &Path) -> tree_sitter::Language {
        let ext = path.extension().and_then(|e| e.to_str()).unwrap_or("");
        match ext.to_ascii_lowercase().as_str() {
            "tsx" => LANGUAGE_TSX.into(),
            _ => LANGUAGE_TYPESCRIPT.into(),
        }
    }
}

fn ts_symbol_name(file: &ParsedFile, node: Node) -> Option<String> {
...
fn ts_is_top_level_variable(node: Node) -> bool {
...
fn ts_decl_range(file: &ParsedFile, symbol_node: Node) -> Option<crate::models::TextRange> {
...
fn ts_visit_symbols(file: &ParsedFile, cursor: &mut TreeCursor, symbols: &mut Vec<Symbol>) {
    loop {
        ...
        if let Some(kind) = symbol_kind {
            if kind == SymbolKind::Variable && !ts_is_top_level_variable(node) {
                // Skip non-top-level variables for now.
            } else if let Some(name) = ts_symbol_name(file, node) {
        ...
        if cursor.goto_first_child() {
            ts_visit_symbols(file, cursor, symbols);
            cursor.goto_parent();
        }
...
fn ts_callee_name(file: &ParsedFile, call_node: Node) -> Option<String> {
    let source = file.source();
    ...
    match function.kind() {
        ...
        "member_expression" => {
            ...
            // Fallback: pick the last property/identifier child.
...
fn ts_attach_call_metadata(file: &ParsedFile, symbols: &mut [Symbol]) {
    if symbols.is_empty() {
        return;
    }
        ...
        loop {
            ...
            if node.kind() == "call_expression" {
                ...
                if let Some(caller_idx) =
                    ts_enclosing_symbol_index(symbols, &file.path, range)
                {
                    if let Some(callee) = ts_callee_name(file, node) {
...
fn ts_context_node_for_ancestor(
    file: &ParsedFile,
    node: Node,
    ...
    let kind = match node.kind() {
        ...
        "method_definition" => Some(SymbolKind::Method),
        "namespace_declaration" | "internal_module" | "module" => Some(SymbolKind::Namespace),
        _ => None,
    }?;

    let name = ts_symbol_name(file, node)?;
...
fn ts_parent_info<'a>(
    ...
    while let Some(node) = current {
        if let Some(ctx) = ts_context_node_for_ancestor(file, node) {
    ...
    // Always include a file-level context as the outermost entry.
...
impl LanguageBackend for TypeScriptBackend {
    ...
    fn parse_file(&self, path: &Path, source: &str) -> BackendResult<ParsedFile> {
        ...
        if tree.root_node().has_error() {
            ...
            return Err(BackendError::new(
                "tree-sitter reported errors while parsing TypeScript source",
            ));
        }
    ...
    fn index_symbols(&self, file: &ParsedFile) -> BackendResult<Vec<Symbol>> {
        ...
        ts_visit_symbols(file, &mut cursor, &mut symbols);
        ts_attach_call_metadata(file, &mut symbols);
        Ok(symbols)
    }
    ...
    fn get_context_snippet(
        ...
        let (parent_node, parent_chain) = ts_parent_info(file, symbol_node);

        let mut context = match kind {
            ContextKind::Decl => {
                if let Some(range) = ts_decl_range(file, symbol_node) {
